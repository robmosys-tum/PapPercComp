...
public:
  ...
  explicit FeatureCloud(float normal_radius = 0.02f, float feature_radius = 0.02f)
    : search_method_xyz_(new SearchMethod), normal_radius_(normal_radius), feature_radius_(feature_radius)
  {
  }
...
public:
  ...
  explicit TemplateAlignment(int nr_samples = 10, float min_sample_distance = 0.05,
                             float max_correspondence_distance = 0.0001, int nr_iterations = 500)
  ...
  void initial_align(FeatureCloud& template_cloud, TemplateAlignment::Result& result)
  {
...
int main(int argc, char* argv[])
{
  ...
  while (ros::ok())
  {
    ...
    *cloud_filtered = *cloud_f;
    ...
    pcl::transformPointCloud(*template_feature_cloud.getPointCloud(), aligned_template_cloud,
