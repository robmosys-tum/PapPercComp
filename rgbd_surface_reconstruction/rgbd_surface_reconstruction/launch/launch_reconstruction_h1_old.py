...
def generate_launch_description():
    ...
    download_indicator = corbs_datasetpath + '/H1_T'
    dataset_url = "http://corbs.dfki.uni-kl.de/suppfiles/Human/H1/H1_Trajectory.zip"
    if not os.path.isfile(download_indicator):
        download_dataset(corbs_datasetpath, dataset_url)
        open(download_indicator, 'a').close()

    download_indicator = corbs_datasetpath + '/H1_D'
    dataset_url = "http://corbs.dfki.uni-kl.de/suppfiles/Human/H1/H1_pre_registereddata.zip"
    if not os.path.isfile(download_indicator):
    ...
    return LaunchDescription([
        Node(
            ...
            parameters=[
                {"data_path": corbs_datasetpath+"/H1_pre_registereddata/",
                 "trajectory_path": corbs_datasetpath+"/H1_Trajectory/"}],
            output='screen',
            emulate_tty=True,
        ),
        Node(
