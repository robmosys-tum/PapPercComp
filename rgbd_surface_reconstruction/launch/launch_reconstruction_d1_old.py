...
def generate_launch_description():
    ...
    download_indicator = corbs_datasetpath + '/D1_T'
    dataset_url = "http://corbs.dfki.uni-kl.de/suppfiles/Desk/D1/D1_Trajectory.zip"
    if not os.path.isfile(download_indicator):
        download_dataset(corbs_datasetpath, dataset_url)
        open(download_indicator, 'a').close()

    download_indicator = corbs_datasetpath + '/D1_D'
    dataset_url = "http://corbs.dfki.uni-kl.de/suppfiles/Desk/D1/D1_pre_registereddata.zip"
    if not os.path.isfile(download_indicator):
    ...
    return LaunchDescription([
        Node(
            ...
            parameters=[
                {"data_path": corbs_datasetpath+"/D1_pre_registereddata/",
                 "trajectory_path": corbs_datasetpath+"/D1_Trajectory/"}],
            output='screen',
            emulate_tty=True,
        ),
        Node(
